...
double Node::resolveLambda(const LambdaDefinitions& definitions, OperationMode mode, std::array<bool, 4>& resolving)
{
...
FailureRates resolvedFailureRates(const LambdaDefinitions& definitions)
{
    ...
    for (int i = 0; i < static_cast<int>(values.size()); ++i)
    {
        ...
        values[i] = Node::resolveLambda(definitions, static_cast<OperationMode>(i), resolving);
    }
    return values;
}
...
Node::NodeKind Node::getNodeKind() const {return configuration.nodeKind;}
void Node::setNodeKind(NodeKind value){configuration.nodeKind = value;}
