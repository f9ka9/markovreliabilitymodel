...
QRectF NodeGraphics::boundingRect() const {return defaultBoundingRect();}

void NodeGraphics::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    const QRectF nodeRect = defaultBoundingRect();
    QColor fillColor = Qt::lightGray;
    if (modelNode)
    {
        switch (modelNode->getNodeKind())
        {
        case NodeKind::Input:
            fillColor = QColor("#BFE8C7");
            break;
        case NodeKind::Output:
            fillColor = QColor("#FFD7A8");
            break;
    ...
    painter->setBrush(isSelected() ? QColor("#ADD8E6") : fillColor);
    painter->drawRoundedRect(nodeRect, 15.0, 15.0);
    painter->setPen(Qt::black);

    QString text = modelNode ? modelNode->getName() : QString("Node");
    if (modelNode && text.isEmpty())
    {
        if (modelNode->getNodeKind() == NodeKind::Input)
            text = "Вход";
        else if (modelNode->getNodeKind() == NodeKind::Output)
            text = "Выход";
    }
    if (text.isEmpty()) text = "Node";

    const QFontMetrics metrics(painter->font());
    text = metrics.elidedText(text, Qt::ElideRight, nodeRect.width() - 8);
