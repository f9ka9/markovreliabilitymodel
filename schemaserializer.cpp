...
bool SchemaSerializer::fromJson(const QJsonDocument& document, EditorSchemaModel& model, QString& errorMessage)
{
    ...
    {
        if (!configurationFromJson(root["defaultNodeConfiguration"].toObject(),
                                   loadedModel.defaultNodeConfiguration,
                                   errorMessage))
            return false;
    }
...
{
    configuration.name = object["name"].toString("Node").trimmed();
    ...
    if (object.contains("lambdaDefinitions"))
    {
        ...
        const QList<QPair<QString, OperationMode>> fields{
            {"transportation", OperationMode::Transportation},
            {"storage", OperationMode::Storage},
            {"functioning", OperationMode::Functioning},
            {"off", OperationMode::Off}
        };
